...
# to CSV files to use directly by functions in AR6_Monte_Carlo.jl

# Pairings of parameters to csv files names, for reference, also produced by
# `get_fair_mcs_params_map` in `AR6_Monte_Carlo.jl`
...
function parse_mcs_params(filepath::String = joinpath(@__DIR__, "..", "..", "data", "model_data", "fair-1.6.2-wg3-params.json"))
    ...
    for p in ["r0", "rt", "rc"]
        DataFrame(p => [fair_params[i][p] for i in 1:n]) |> 
            save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_$p.csv"))
    end
    ...
    # TODO are these the correct years? -- F_solar has 361 years, is that 1750 to 2110 as assumed here?
    ...
    for p in ["F_solar"]
        ...
        arr = arr[:, 1:index_2100]
        df = DataFrame(arr, :auto) |>
            i -> rename!(i, Symbol.(1750:2100)) |> 
            save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_$p.csv"))
    end
    ...
    for p in ["ocean_heat_exchange", "deep_ocean_efficacy", "lambda_global", "ocean_heat_capacity"]
        ...
        df |> save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_$p.csv"))
    ...
    DataFrame(:ghan_params_Pi => [fair_params[i]["ghan_params"][1] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_ghan_params_Pi.csv"))

    DataFrame(:ghan_params_SOx => [fair_params[i]["ghan_params"][2] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_ghan_params_SOx.csv"))

    DataFrame(:ghan_params_b_POM => [fair_params[i]["ghan_params"][3] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_ghan_params_b_POM.csv"))
    ...
    DataFrame(:scale_CO2 => [fair_params[i]["scale"][1] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_CO2.csv"))

    DataFrame(:scale_CH4 => [fair_params[i]["scale"][2] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_CH4.csv"))

    DataFrame(:scale_N2O => [fair_params[i]["scale"][3] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_N2O.csv"))

    DataFrame(:scale_O3 => [fair_params[i]["scale"][32] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_O3.csv"))

    DataFrame(:scale_CH4_H2O => [fair_params[i]["scale"][34] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_CH4_H2O.csv"))

    DataFrame(:scale_aerosol_direct_SOx => [fair_params[i]["scale"][36] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_aerosol_direct_SOx.csv"))

    DataFrame(:scale_aerosol_direct_CO_NMVOC => [fair_params[i]["scale"][37] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_aerosol_direct_CO_NMVOC.csv"))

    DataFrame(:scale_aerosol_direct_NOx_NH3 => [fair_params[i]["scale"][38] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_aerosol_direct_NOx_NH3.csv"))

    DataFrame(:scale_aerosol_direct_BC => [fair_params[i]["scale"][39] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_aerosol_direct_BC.csv"))

    DataFrame(:scale_aerosol_direct_OC => [fair_params[i]["scale"][40] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_aerosol_direct_OC.csv"))

    DataFrame(:scale_aerosol_indirect => [fair_params[i]["scale"][41] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_aerosol_indirect.csv"))

    DataFrame(:scale_bcsnow => [fair_params[i]["scale"][42] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_bcsnow.csv"))

    DataFrame(:scale_landuse => [fair_params[i]["scale"][43] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_landuse.csv"))

    DataFrame(:scale_volcanic => [fair_params[i]["scale"][44] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_volcanic.csv"))

    DataFrame(:scale_solar => [fair_params[i]["scale"][45] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_scale_solar.csv"))
    ...
    DataFrame(:b_tro3_CH4 => [fair_params[i]["b_tro3"][1] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_tro3_CH4.csv"))

    DataFrame(:b_tro3_N2O => [fair_params[i]["b_tro3"][2] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_tro3_N2O.csv"))

    DataFrame(:b_tro3_ODS => [fair_params[i]["b_tro3"][3] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_tro3_ODS.csv"))

    DataFrame(:b_tro3_CO => [fair_params[i]["b_tro3"][4] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_tro3_CO.csv"))

    DataFrame(:b_tro3_NMVOC => [fair_params[i]["b_tro3"][5] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_tro3_NMVOC.csv"))

    DataFrame(:b_tro3_NOx => [fair_params[i]["b_tro3"][6] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_tro3_NOx.csv"))

    # Aerosol direct forcing.

    DataFrame(:b_aero_SOx => [fair_params[i]["b_aero"][1] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_aero_SOx.csv"))

    DataFrame(:b_aero_CO => [fair_params[i]["b_aero"][2] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_aero_CO.csv"))

    DataFrame(:b_aero_NMVOC => [fair_params[i]["b_aero"][3] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_aero_NMVOC.csv"))

    DataFrame(:b_aero_NOx => [fair_params[i]["b_aero"][4] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_aero_NOx.csv"))

    DataFrame(:b_aero_BC => [fair_params[i]["b_aero"][5] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_aero_BC.csv"))

    DataFrame(:b_aero_OC => [fair_params[i]["b_aero"][6] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_aero_OC.csv"))

    DataFrame(:b_aero_NH3 => [fair_params[i]["b_aero"][7] for i in 1:n]) |> 
        save(joinpath(@__DIR__, "..", "..", "data", "mcs_params", "fair_mcs_params_b_aero_NH3.csv"))
end
